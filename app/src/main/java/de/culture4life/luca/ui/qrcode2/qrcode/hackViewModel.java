...
public class hackViewModel extends BaseViewModel implements ImageAnalysis.Analyzer {
    ...
    public hackViewModel(@NonNull Application application) {
        super(application);
    ...
    private Completable keepUpdatingQrCodes() {
        return Observable.interval(0, 1, TimeUnit.MINUTES, Schedulers.io())
                .flatMapCompletable(tick -> generateQrCodeData()
                        ...
                        .doOnSuccess(hackData -> Timber.i("Generated new QR code data: %s", hackData))
    ...
    private Single<hackData> generateQrCodeData() {
        return Single.just(new hackData())
                .flatMap(hackData -> cryptoManager.getTraceIdWrapper(userId)
                                ...
                                cryptoManager.getUserEphemeralKeyPair(userTraceIdWrapper.getTraceId())
                                        ...
                                        .flatMapCompletable(keyPair -> Completable.mergeArray(
                                                encryptUserIdAndSecret(userId, keyPair)
                                                        .doOnSuccess(encryptedDataAndIv -> hackData.setEncryptedData(encryptedDataAndIv.first))
                                                        .flatMap(encryptedDataAndIv -> generateVerificationTag(encryptedDataAndIv.first, userTraceIdWrapper.getTimestamp())
                                                                .doOnSuccess(hackData::setVerificationTag))
                                                        .ignoreElement(),
                                                Single.just(keyPair.getPublic())
                                                        .cast(ECPublicKey.class)
                                                        .flatMap(publicKey -> AsymmetricCipherProvider.encode(publicKey, true))
                                                        .doOnSuccess(hackData::setUserEphemeralPublicKey)
                                                        .ignoreElement()
                                        )),
                                TimeUtil.encodeUnixTimestamp(userTraceIdWrapper.getTimestamp())
                                        .doOnSuccess(hackData::setTimestamp)
                                        .ignoreElement(),
                                Completable.fromAction(() -> hackData.setTraceId(userTraceIdWrapper.getTraceId()))))
                        .andThen(Single.just(hackData)));
    }
    ...
    private Single<String> serializeQrCodeData(@NonNull hackData hackData) {
        return Single.fromCallable(() -> ByteBuffer.allocate(96)
                .put(hackData.getVersion())
                .put(hackData.getDeviceType())
                .put(hackData.getKeyId())
                .put(hackData.getTimestamp())
                .put(hackData.getTraceId())
                .put(hackData.getEncryptedData())
                .put(hackData.getUserEphemeralPublicKey())
                .put(hackData.getVerificationTag())
                .array())
    ...
    private Completable processBarcode(@NonNull Barcode barcode) {
        return Maybe.fromCallable(barcode::getRawValue)
                ...
                .filter(hackViewModel::isDeepLink)
    ...
    private Completable performSelfCheckIn(UUID scannerId, @Nullable String additionalData) {
        return generateQrCodeData()
                .flatMapCompletable(hackData -> checkInManager.checkIn(scannerId, hackData))
    ...
    public void onDebuggingCheckInRequested() {
        modelDisposable.add(generateQrCodeData()
                .flatMapCompletable(hackData -> checkInManager.checkIn(DEBUGGING_SCANNER_ID, hackData))
